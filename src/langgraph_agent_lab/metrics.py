...
def summarize_metrics(
    ...
    ``resume_success`` is supplied by the caller once it has actually verified a
    checkpoint replay (see ``cli.run_scenarios`` / ``cli.recovery-demo``); it stays
    False unless that evidence exists.
    """
    if not items:
